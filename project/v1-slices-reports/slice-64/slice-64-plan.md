# Slice 64 Plan v3 (Final) - Same-SHA CI Evidence Remediation Continuation for `config_init|n/a` (Evidence-Only, No Route Mutation)
...
## 2) Goal / Objective
Continue fail-closed same-SHA CI evidence collection for `config_init|n/a` on the current local SHA and record an explicit evidence outcome, while preserving all governance truth unchanged.
...
- Report artifact workflow for this slice (`plan`, `implementation`, `audit`).

### Out of scope
- Any mutation of governance-truth files:
...
- Planning worker: `project/v1-slices-reports/slice-64/slice-64-plan.md` only
- Implementation worker: `project/v1-slices-reports/slice-64/slice-64-implementation.md` only
- Audit worker: `project/v1-slices-reports/slice-64/slice-64-audit.md` only
...
5. Uniqueness proof command output is recorded and exact:
   - `decision_token_match_count=1`
   - `same_sha_branch_match_count=1`
   - Proof source commands must be included in implementation evidence (`Select-String` or `rg` equivalents).
...
8. Protected-file diff proof is empty for all out-of-scope governance files and `WORK_ITEMS.md` prior to post-audit update.
...
11. Audit report returns explicit PASS before any `WORK_ITEMS.md` change.

## 6) Binary Success Condition
Slice 64 is successful only if exactly one same-SHA branch marker and exactly one decision token are recorded with uniqueness proof outputs, all protected governance files remain unchanged, governance checks pass, and audit returns PASS before any `WORK_ITEMS.md` update.
...
- Slice 65 (conditional fork, no implicit mutation in Slice 64):
...
rg -n "decision_gate:" project/v1-slices-reports/slice-64/slice-64-implementation.md
rg -n "same_sha_branch:" project/v1-slices-reports/slice-64/slice-64-implementation.md
...
- Plan: `project/v1-slices-reports/slice-64/slice-64-plan.md`
- Implementation: `project/v1-slices-reports/slice-64/slice-64-implementation.md`
- Audit: `project/v1-slices-reports/slice-64/slice-64-audit.md`

## 13) Split-if-too-large Rule
Stop immediately and split this work into follow-on slices if any of the following occurs:
- Scope expands beyond single-row target (`config_init|n/a`) or attempts multi-row evidence work.
- Any governance-truth mutation becomes necessary (`route-fence`, parity, workflow index, verification contract, CI workflow).
- Any runtime or test edits under `src/` or `tests/` become necessary.
- Tooling/auth setup beyond evidence capture/reporting is required (for example, environment provisioning, dependency installation, or credential/bootstrap work).
If a split is triggered, this slice remains evidence-only/no-mutation and the additional work is deferred to the next explicitly planned slice.
